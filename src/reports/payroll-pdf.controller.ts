import { Controller, Get, Query, Res, HttpException, HttpStatus, UseGuards } from '@nestjs/common';
import { Response } from 'express';
import { PayrollPdfService } from './payroll-pdf.service';
import { JwtAuthGuard } from '../auth/jwt-auth.guard';
import { RolesGuard } from '../auth/roles.guard';
import { Roles } from '../auth/roles.decorator';

@Controller('reports')
@UseGuards(JwtAuthGuard, RolesGuard)
export class PayrollPdfController {
  constructor(private readonly payrollPdfService: PayrollPdfService) {}

  @Get('payroll-pdf')
  @Roles('HR', 'FINANCE', 'ADMIN')
  async downloadPayroll(@Query('runId') runId: string, @Res() res: Response) {
    try {
      const buffer = await this.payrollPdfService.generateMonthlyPayrollPDF(runId);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="payroll.pdf"`);
      return res.send(buffer);
    } catch (e) {
      throw new HttpException(e.message || 'Failed to generate payroll PDF', HttpStatus.BAD_REQUEST);
    }
  }
}